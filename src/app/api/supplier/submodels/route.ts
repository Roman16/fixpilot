...
import { parseSubModels } from '@/lib/supplierParser';
...
export async function GET(req: Request) {
    try {
        ...
        if (!modelId) return NextResponse.json({ message: 'modelId обовʼязковий' }, { status: 400 });
        ...
        return NextResponse.json({ data: parseSubModels(js) });
    } catch (error: any) {
